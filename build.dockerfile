...
RUN touch /usr/bin/cc  && mv /usr/bin/cc /usr/bin/cc.bak && \
...
# Compile protobuf
RUN git clone https://github.com/protocolbuffers/protobuf.git -b $PROTOBUF_VERSION /tmp/protobuf --depth 1 --recurse-submodules && \
...
# Install Wasm tools
RUN wget https://github.com/WebAssembly/binaryen/releases/download/version_$WASM_BINDGEN_VERSION/binaryen-version_$WASM_BINDGEN_VERSION-x86_64-linux.tar.gz -O /tmp/binaryen.tar.gz && \
